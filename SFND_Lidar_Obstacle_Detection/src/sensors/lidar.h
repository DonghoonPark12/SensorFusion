...
struct Ray
{
	...
	{
		/*
			Lidar will use Ray to sense its surrounding by doing ray casting.
		*/
...
struct Lidar
{
	...
	{
		/*
			Generate point cloud for us.
			We will store results in a PointCloud pointer object 'pcl::PointCloud<pcl::PointXYZ>::Ptr'
			(Ptr is a pointer --> contain memory address of my point cloud object)
			()

			Type of point for the PointCloud pcl::PointXYZ
			'renderRays' function will be called.
		*/
		cloud->points.clear();
