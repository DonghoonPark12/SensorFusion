...
int main ()
{
	...
	KdTree *tree = new KdTree; //struct pointer
	...
	*/
  	...
  	//
  	std::vector<std::vector<int>> clusters = euclideanCluster(points, tree, 3.0); //2 dimensional vector
  	//
	...
	std::vector<Color> colors = {Color(1,0,0), Color(0,1,0), Color(0,0,1)};
  	...
  	{
  		pcl::PointCloud<pcl::PointXYZ>::Ptr clusterCloud(new pcl::PointCloud<pcl::PointXYZ>());//allocation cluster
  		for(int indice: cluster)// There would be 3 clusters.
