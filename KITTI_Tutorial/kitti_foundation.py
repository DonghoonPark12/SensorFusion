...
class Kitti:
    ...
    def __load_from_bin(self):
        ...
        if self.__frame_type in velo_files:
            ...
            # shape : tuple(123949,4) --> extract (x,y,z) so, (123949,3)
            # max : 77.797
            # min : -56.869
        else:
...
class Kitti_util(Kitti):
    ...
    def __point_matrix(self, points):
        ...
        xyz_ = xyz_.T # size : 104724, shape : (3, 34908)
    ...
    def __velo_2_img_projection(self, points):
        ...
        for i in range(xyz_v.shape[1]):
            xyz_v[:3, i] = np.matmul(RT_, xyz_v[:, i]) # xyz_v shape : (4, 34908)
        ...
        xyz_c =  [y_1 , y_2 , .. ]
                 [z_1 , z_2 , .. ]
        """
        xyz_c = np.delete(xyz_v, 3, axis=0) # xyz_c shape : (3, 34908)
        ...
        xy_i =   [s_1*y_1 , s_2*y_2 , .. ]        ans =   [x_1 , x_2 , .. ]
                 [  s_1   ,   s_2   , .. ]                [y_1 , y_2 , .. ]
        """
        xy_i = xyz_c[::] / xyz_c[::][2] #What is the meaning of this??
